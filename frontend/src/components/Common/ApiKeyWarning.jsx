...
const ApiKeyWarning = ({ onValidationChange }) => {
  ...
  const checkApiKeys = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000'}/api/v1/api-keys/status`, {
        credentials: 'include'
      });
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
              <div className={`p-3 rounded-lg border ${keyStatus?.gemini?.status === 'valid'
                  ? 'border-green-200 bg-green-50 dark:bg-green-800 dark:bg-green-900/20'
                  : 'border-amber-300 bg-amber-100 dark:bg-amber-700 dark:bg-amber-900/20'
                }`}>
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`text-sm font-medium ${keyStatus?.gemini?.status === 'valid'
                ...
                <p className={`text-xs mt-1 ${keyStatus?.gemini?.status === 'valid'
              ...
              <div className={`p-3 rounded-lg border ${keyStatus?.youtube?.status === 'valid'
                  ? 'border-green-200 bg-green-50 dark:bg-green-800 dark:bg-green-900/20'
                  : 'border-amber-300 bg-amber-100 dark:bg-amber-700 dark:bg-amber-900/20'
                }`}>
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`text-sm font-medium ${keyStatus?.youtube?.status === 'valid'
                      ? 'text-green-800 dark:text-green-200'
                      : 'text-amber-800 dark:text-amber-200'
                    }`}>
                    📺 YouTube Data API
                ...
                <p className={`text-xs mt-1 ${keyStatus?.youtube?.status === 'valid'
                    ? 'text-green-600 dark:text-green-300'
                    : 'text-amber-600 dark:text-amber-300'
                  }`}>

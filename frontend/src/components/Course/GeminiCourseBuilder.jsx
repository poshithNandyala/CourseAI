...
export const GeminiCourseBuilder = () => {
  ...
  const [showGenerationSidebar, setShowGenerationSidebar] =
    useState(false);
  const [isGenerationMinimized, setIsGenerationMinimized] =
    useState(false);
  const [realGenerationSteps, setRealGenerationSteps] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success(
        `🎉 Course generated! Found ${result.metadata.videoCount} real YouTube videos across ${result.metadata.subtopicsCount} topics.`
      );
    } catch (error) {
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3 pt-3">
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${courseSettings.includeQuizzes
                        ? "bg-brand-500"
                        : "bg-gray-300 dark:bg-gray-600"
                        }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${courseSettings.includeQuizzes
                          ? "translate-x-6"
                          : "translate-x-1"
                          }`}
                      />
              ...
              <div className="flex border-b border-gray-200 dark:border-gray-800">
                {[
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center space-x-2 px-6 py-4 font-medium transition-all duration-200 ${activeTab === tab.id
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {generatedCourse.lessons.map((lesson, index) => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl font-medium transition-all duration-200 ${selectedLessonIndex === index

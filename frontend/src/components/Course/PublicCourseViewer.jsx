...
export const PublicCourseViewer = () => {
  const { id } = useParams();
  ...
  const loadPublicCourse = async () => {
    ...
    try {
      ...
      console.log('🌍 Course ID value:', id);
      
      // The backend getCourseById returns both course and lessons
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000/api/v1'}/courses/${id}`, {
        method: 'GET',
        headers: {

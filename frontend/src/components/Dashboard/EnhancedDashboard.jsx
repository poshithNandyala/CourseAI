...
import {
  ...
  unpublishCourse,
  deleteCourse,
  fetchUserCoursesFromUserRoute,
  fetchMyPublishedCoursesLikes,
...
export const EnhancedDashboard = () => {
  ...
  // Remove the user check since ProtectedRoute handles this
  ...
  }, [user?.id]); // Use user.id instead of entire user object
  ...
  const loadTotalLikes = async () => {
    ...
    try {
      const likes = await fetchMyPublishedCoursesLikes();
      // Handle the array of courses with likes
      const totalLikesCount = Array.isArray(likes)
        ? likes.reduce((sum, course) => sum + (course.likes || 0), 0)
        : 0;
      setTotalLikes(totalLikesCount);
    } catch (e) {
      console.error("Error loading total likes:", e);
      setTotalLikes(0);
    } finally {
      setLoadingLikes(false);
    }
  ...
  const handlePublishToggle = async (courseId, isCurrentlyPublished) => {
    try {
      console.log(
        `🔄 Toggling publish status for course ${courseId} to ${!isCurrentlyPublished}`
      );
      ...
      if (isCurrentlyPublished) {
        ...
        toast.success(
          "Course published successfully! It is now visible to others."
        );
      }
  ...
  const handleDeleteCourse = async (courseId, courseTitle) => {
    // Note: The confirm() dialog might not work as expected in all environments.
    // It's recommended to replace this with a custom modal component for a better user experience.
    if (
      !confirm(
  ...
  const handleViewCourse = (courseId, isPublished) => {
          ...
          <div className="text-center py-12">
            <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-lg border border-gray-200 dark:border-gray-800">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                You need to be signed in to access your course dashboard and
                manage your content.
              </p>
              <Link
                to="/signin"
        ...
        >
          {loading ? (
            ...
            <div className="text-center py-12 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                Create your first course with AI-powered content generation
              </p>
              <button
                onClick={handleCreateCourse}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map((course, index) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: index * 0.1 }}
                  className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-800 hover:shadow-xl hover:border-brand-200 dark:hover:border-brand-800 transition-all duration-200"
                >
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center space-x-2">
                        <span
                          className={`px-2 py-1 text-xs rounded-full font-medium ${course.is_published
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className={`flex-1 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 flex items-center justify-center space-x-1 ${course.is_published

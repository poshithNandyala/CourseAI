...
export const QuizComponent = ({ 
  ...
  const calculateResults = () => {
    ...
    return {
      ...
      total: length,
      percentage,
      easyScore > 0 ? Math.round((easyCorrect / easyTotal) * 100) ,
      mediumScore > 0 ? Math.round((mediumCorrect / mediumTotal) * 100) ,
      hardScore > 0 ? Math.round((hardCorrect / hardTotal) * 100) ,
      timeSpent
    };
  };
    ...
    return (
      ...
      >
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-gray-900 dark-white mb-2">Quiz Complete</h2>
          <p className="text-gray-600 dark-gray-400">Here are your results for {title}</p>
        </div>
        ...
        <div className="grid md-cols-3 gap-4 mb-6">
          <div className="bg-green-50 dark-green-900/20 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-green-600 dark-green-400">
              {results.easyScore}%
            </div>
            <div className="text-sm text-green-700 dark-green-300">Easy Questions</div>
          </div>
          <div className="bg-yellow-50 dark-yellow-900/20 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-yellow-600 dark-yellow-400">
              {results.mediumScore}%
            </div>
            <div className="text-sm text-yellow-700 dark-yellow-300">Medium Questions</div>
          </div>
          <div className="bg-red-50 dark-red-900/20 rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-red-600 dark-red-400">
              {results.hardScore}%
            </div>
            <div className="text-sm text-red-700 dark-red-300">Hard Questions</div>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-8">
          <h3 className="font-semibold text-gray-900 dark-white mb-4">Question Review</h3>
          {questions.map((question, index) => {
            const userAnswer = selectedAnswers[index];
            const isCorrect = userAnswer === question.correctAnswer;
            
            return (
              <div key={index} className="p-4 bg-gray-50 dark-gray-800 rounded-xl">
                <div className="flex items-start space-x-3">
                  <div className={`p-1 rounded-full ${isCorrect ? 'bg-green-100 dark-green-900/30' : 'bg-red-100 dark-red-900/30'}`}>
                    {isCorrect ? (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm space-y-1">
                      <div className="text-gray-600 dark-gray-400">
                        Your answer: <span className={isCorrect ? 'text-green-600 dark-green-400' : 'text-red-600 dark-red-400'}>{userAnswer}</span>
                      </div>
                      {isCorrect && (
                        <div className="text-gray-600 dark-gray-400">
                          Correct answer: <span className="text-green-600 dark-green-400">{question.correctAnswer}</span>
                        </div>
                      )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark-white mb-2">{title}</h2>
          <div className="flex items-center space-x-4 text-sm text-gray-500 dark-gray-400">
            <span>Question {currentQuestionIndex + 1} of {questions.length}</span>
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
              currentQuestion.difficulty === 'easy' ? 'bg-green-100 dark-green-900/30 text-green-700 dark-green-300' .difficulty === 'medium' ? 'bg-yellow-100 dark-yellow-900/30 text-yellow-700 dark-yellow-300' :
              'bg-red-100 dark-red-900/30 text-red-700 dark-red-300'
            }`}>

...
class OpenAIService {
  ...
  async generateCourse(request)<GeneratedCourseContent> {
    if (this.apiKey) {
  ...
  private buildCoursePrompt(request) {
    return `
...
Please provide a detailed course structure including. Course title and description
...
Format the response structured outline that can be easily parsed. Focus on practical, real-world applications and ensure the content is engaging and progressive in difficulty.
...
The course should be suitable for ${request.difficulty} learners and include modern best practices and industry-relevant skills.
  ...
  private generateMockCourse(request) {
    ...
    const courseData = {
      ...
      description: `Master ${request.topic} with this comprehensive ${request.difficulty} course. Learn through hands-on projects, real-world examples, and industry best practices. Perfect for ${request.difficulty === 'beginner' ? 'those new to' .difficulty === 'intermediate' ? 'those with some experience in' : 'experienced developers wanting to master'} ${request.topic}.`,
      targetAudience.getTargetAudience(request.topic, request.difficulty),
      prerequisites.getPrerequisites(request.topic, request.difficulty),
      learningObjectives.getLearningObjectives(request.topic, request.difficulty),
      courseOutline.generateCourseOutline(request.topic, request.difficulty, request.duration),
      assessments.generateAssessments(request.topic, request.difficulty),
      projects.generateProjects(request.topic, request.difficulty, request.includeProjects)
    };

    return courseData;
  }

  private getTargetAudience(topic, difficulty) {
    const audiences = {
    ...
    return audiences[difficulty typeof audiences];
  }

  private getPrerequisites(topic, difficulty) {
  ...
  private getLearningObjectives(topic, difficulty) {
    const topicLower = topic.toLowerCase();
  ...
  private generateCourseOutline(topic, difficulty, duration) {
  ...
  private generateAssessments(topic, difficulty) {
    return [
      {
        type: 'quiz',
  ...
  private generateProjects(topic, difficulty, includeProjects) {
    ...
    if (topicLower.includes('react')) {
      return [
        ...
        },
        {
          ...
          deliverables: ['Working task manager', 'Data persistence', 'User-friendly 
      ];
    }

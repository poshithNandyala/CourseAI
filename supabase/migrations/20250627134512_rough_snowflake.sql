/*
  # Fix Public Course Access Policies

  1. Security Changes
    - Drop and recreate policies to avoid conflicts
    - Enable public access to published courses
    - Allow public viewing of lessons and comments for published courses
    - Create course likes functionality

  2. New Tables
    - `course_likes` table for tracking user likes

  3. Functions
    - Functions to manage course likes and ratings
*/
...
-- Enable RLS on course_likes if not already enabled
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'course_likes' 
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    AND course_likes.user_id = toggle_course_like.user_id
  ) INTO like_exists;
  ...
  ELSE
    ...
    VALUES (toggle_course_like.course_id, toggle_course_like.user_id);
